...
/********** ARRAY2-system **********/
/*  Changes are made in following procedures for ARRAY2-system:
sweep
saveimage
loadimage
delassoc
...
setup
repl  // 'quit'-variable
buildstring  - changed for x86 machines
*MakeFilename
...
// ESP32-2432S028R - ESP32-WROOM board with TFT and touchscreen


#if defined(gfxsupport)

#define COLOR_WHITE 0x00ffff //ST77XX_WHITE
#define COLOR_BLACK 0x000000 //ST77XX_BLACK
...
#if defined(DEF_ARRAY2)
void SDReadN (File file, char *data, int size) {
    for(int i=0;i<size;i++) {
        *data++ = file.read();
    };
}
void SDWriteN (File file, char *data, int size) {
    for(int i=0;i<size;i++) {
        file.write(*data++);
     };
}
...
#if defined(DEF_ARRAY2)
void FSReadN (File file, char *data, int size) {
    file.read(data, size);
}
void FSWriteN (File file, char *data, int size) {
...
#if defined(DEF_ARRAY2)
void EpromReadN (int *addr, char *data, int size) {
    for(int i=0;i<size;i++) {
        *data = EEPROM.read((*addr)++));
        data++ ;
    };
}
void EpromWriteN (int *addr, char *data, int size) {
    for(int i=0;i<size;i++) {
...
/********** ARRAY2-system **********/
  for (unsigned int i=0; i<imagesize; i++) {
    ...
    if(obj->type == ARRAY2)
    {
      array_desc_t *descriptor = (array_desc_t*)obj->pointer ;
      sprintf(cBuffer,"\nWrite cell:%d,  array size %d", i, (descriptor->size*descriptor->element_size)) ;
      pfstring(cBuffer, pserial);
      SDWriteN(file, (char*)obj->pointer, (descriptor->size*descriptor->element_size)+sizeof(array_desc_t));
    }
  }
/********** ARRAY2-system **********/
#endif
...
/********** ARRAY2-system **********/
  for (unsigned int i=0; i<imagesize; i++) {
    ...
    {
        array_desc_t *descriptor = (array_desc_t*)obj->pointer ;
        FSWriteN(file, (char*)obj->pointer, (descriptor->size*descriptor->element_size)+sizeof(array_desc_t));
    }
  }
/********** ARRAY2-system **********/
#endif
...
/********** ARRAY2-system **********/
  for (unsigned int i=0; i<imagesize; i++) {
    ...
    {
        array_desc_t *descriptor = (array_desc_t*)obj->pointer ;
        EpromWriteN(&addr, (char*)obj->pointer, (d)escriptor->size*descriptor->element_size)+sizeof(array_desc_t)_;
    }
  }
/********** ARRAY2-system **********/
#endif
...
/********** ARRAY2-system **********/
  for (unsigned int i=0; i<imagesize; i++) {
    ...
    if(obj->type == ARRAY2)
    {
      ...
      SDReadN(file, (char*)&descriptor, sizeof(array_desc_t));
      int fullsize = (descriptor.size*descriptor.element_size) ;
#if defined(BOARD_HAS_PSRAM)
      obj->pointer = (pointer_t)ps_malloc(fullsize + sizeof(array_desc_t));
#else
      obj->pointer = (pointer_t)malloc(fullsize + sizeof(array_desc_t));
#endif
      *(array_desc_t*)(obj->pointer) = descriptor ;
      sprintf(cBuffer,"\nRead cell:%d,  array size %d", i, fullsize) ;
      pfstring(cBuffer, pserial);
      SDReadN(file, ((char*)obj->pointer) + sizeof(array_desc_t), fullsize);
    }
  }
/********** ARRAY2-system **********/
#endif
...
/********** ARRAY2-system **********/
  for (unsigned int i=0; i<imagesize; i++) {
    ...
    {
      array_desc_t descriptor ;
      FSReadN(file, (char*)&descriptor, sizeof(array_desc_t));
      int fullsize = descriptor.size*descriptor.element_size ;
#if defined(BOARD_HAS_PSRAM)
      obj->pointer = (pointer_t)ps_malloc(fullsize + sizeof(array_desc_t));
#else
...
#endif
      *(array_desc_t*)(obj->pointer) = descriptor ;
      sprintf(cBuffer,"\nRead cell:%d  size %d", i, fullsize) ;
      pfstring(cBuffer, pserial);
      FSReadN(file, ((char*)obj->pointer) + sizeof(array_desc_t), fullsize);
    }
  }
/********** ARRAY2-system **********/
#endif
...
/********** ARRAY2-system **********/
  for (unsigned int i=0; i<imagesize; i++) {
    ...
    {
      array_desc_t descriptor ;
      EpromReadN(file, (char*)&descriptor, sizeof(array_desc_t));
      int fullsize = descriptor.size*descriptor.element_size ;
#if defined(BOARD_HAS_PSRAM)
      obj->pointer = (pointer_t)ps_malloc(fullsize + sizeof(array_desc_t));
#else
      ...
      *(array_desc_t*)(obj->pointer) = descriptor ;
      sprintf(cBuffer,"\nRead cell:%d  size %d", i, fullsize) ;
      pfstring(cBuffer, pserial);
      EpromReadN(&addr, ((char*)obj->pointer) + sizeof(array_desc_t), fullsize);
    }
  }
/********** ARRAY2-system **********/
#endif
...
#if defined(DEF_ARRAY2)
/********** ARRAY2-system **********/
  object **loc = place(args, env, &bit, NULL);
/********** ARRAY2-system **********/
#else
...
#if defined(DEF_ARRAY2)
/********** ARRAY2-system **********/
  if(var->type == ARRAY2 ) delarray2(var) ;
/********** ARRAY2-system **********/
#endif
...
#if defined(DEF_ARRAY2)
/********** ARRAY2-system **********/
  else if (array2p(form)) printarray2(form, pfun);
/********** ARRAY2-system **********/
#endif
